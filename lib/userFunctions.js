...
class Users {
  ...
  async getSpecificUser(nickname) {
    const { data: user } = await axios.get(`/users/${nickname}`);
  ...
  async updateAvatarPicture(imageFormData) {
    const {
      data: { avatarURL },
    } = await axios.patch(`/users/avatar`, imageFormData);
